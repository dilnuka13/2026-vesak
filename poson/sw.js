const CACHE_NAME = 'poson-finance-v3';
...
messaging.onBackgroundMessage((payload) => {
  const notificationTitle = payload.notification?.title || "Poson System Update";
  const notificationOptions = {
    body: payload.notification?.body || "New activity detected.",
    icon: '../poson_logo.png',
    badge: '../poson_logo.png',
    data: { url: '/poson/#dashboard' }
  };
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = event.notification.data?.url || '/poson/';
  event.waitUntil(
